...
if(mysqli_num_rows($result)>0){
    ...
    if($status==1){
        ...
        //Insert into Table
        $sql="INSERT INTO $tbname(emri, mbiemri, mosha, gjinia, email, cel, username, pasword)
        VALUES ('$emri', '$mbiemri', '$mosha', '$gjinia', '$email', '$cel', '$user', '$pasword')";

    $fileStudent = fopen("$emri.$mbiemri.php", "w") or die("Unable to open file!");
    $txt = '<!DOCTYPE html>
    <html>
    <head>
    <style>
    ...
    <h2 style="text-shadow: 2px 2px 5px red; text-transform: uppercase; font-family: "Lucida Console", "Courier New", monospace;">Zgjidhni kursin/programin ku merrni pjesë!!</h2>
      <form id="kursi" action="phpStudenti.php" method="post">
        ...
        <form id="programi" action="phpProgrami.php" method="post">
    ...
    fwrite($fileStudent, $txt);
    fclose($fileStudent); 
    
        if(mysqli_query($conn, $sql)){
            echo "Urime! Sapo jeni regjistruar si student!";
